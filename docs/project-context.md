...
- [x] IntakeForm.tsx moved to private submodule `a1-website-internal` — mounted at `internal/`
- [x] Brand assets (logo, favicon) moved to private submodule — copied to `public/` by prebuild script
- [x] All internal docs (PROJECT_CONTEXT, COPY_MANUAL, UBUNTU_SERVER_SETUP, DOMAIN_WIRING) moved to private submodule
- [x] CLAUDE.md and .claude/ settings moved to private submodule — stub pointer in public repo
- [x] CORS fixed — FALLBACK_ORIGINS hardcoded in API so restarts never block the site
- [x] API file timestamps switched from UTC to Pacific time (America/Los_Angeles)
- [x] Cloudflare "Always Use HTTPS" enabled — http:// requests redirect to https:// at the edge
- [x] http:// origin variants added to CORS allowlist as belt-and-suspenders backup
